...
class PathHandler:
    def __init__(self, pred_dir:Path, targ_dir:Path, img_id:str, test_hold:str, disaster:str):
        """
        Args:
            ...
            disaster  (str) : type of disaster i-e social-fire etc
        """
        ...
        #['socal-fire', '00000000', 'pre', 'disaster', 'target']
        self.lp = pred_dir/f"{disaster}_{img_id}_pre_disaster_prediction.png" # localization prediction
        self.dp = pred_dir/f"{disaster}_{img_id}_post_disaster_prediction.png" # damage prediction
        self.lt = targ_dir/f"{disaster}_{img_id}_pre_disaster_target.png" # localization target
        self.dt = targ_dir/f"{disaster}_{img_id}_post_disaster_target.png" # damage target
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        ...
        for path in self.targ_dir.glob('*.png'):
            ...
            #['socal-fire', '00000000', 'pre', 'disaster', 'target']
            disaster , img_id, loc_dmg, _ , target = path.name.rstrip('.png').split('_')
            assert loc_dmg in ['pre', 'post'], f"target filenames must have 'pre' or 'post' in filename, got {path}"
            assert target == 'target', f"{target} should equal 'target' when getting path handlers"
            test_hold = 'test' # forcing to use only test and NOT holdout
            if loc_dmg == 'pre': # pre or post is fine here
                self.path_handlers.append(PathHandler(self.pred_dir, self.targ_dir, img_id, test_hold,disaster))
...
#python xview2_Inference.py --pred_dir test\predictions --targ_dir test\targets --out_fp results\out_110.json  
