...
def get_dataset(temporal = 'post', data_path = 'train/', use_transform = True):
...
def main(args):
    ...
    print(np.shape(masks),len(masks))
    print(np.unique(masks[0]))   # Always [0,1]

    labels = labels[0]['labels'].numpy()
    print(labels)


    tmp = np.zeros((1024,1024),dtype=np.uint8)

    for i in range(len(masks)):
        tmp[masks[i]==1] = labels[i]
    mask = tmp
    ...
    target = Image.open('socal-fire_00000166_post_disaster_target.png')
